...
class WC_Parsian_Payment_Gateway extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_description = __('پرداخت امن با کارت‌های عضو شتاب از طریق بانک پارسیان', 'parsian-gateway');
        ...
        add_action('woocommerce_api_wc_' . strtolower($this->id), [$this, 'handle_callback']);
    }
    ...
    public function process_payment($order_id) {
        ...
        $callback_url = WC()->api_request_url('wc_' . strtolower($this->id));
        ...
        $response = Parsian_API::send_payment_request($data);

        if (isset($response['Status']) && intval($response['Status']) === 0 && !empty($response['Token'])) {
            $token = sanitize_text_field($response['Token']);
            $order->update_status('pending', __('در انتظار پرداخت', 'parsian-gateway'));
            $order->update_meta_data('_parsian_token', $token);
            $order->save();
            ...
            $error_message = Parsian_Utils::get_status_message($response['Status'] ?? -32768);
    ...
    public function handle_callback() {
        ...
        if ($status == 0 && $token) {
            $verification = Parsian_API::verify_payment($this->terminal_id, $token);

            if ($verification['Status'] == 0) {
                $rrn = $verification['RRN'] ?? '';
                $success_message = str_replace('{RRN}', esc_html($rrn), $this->success_message);
    ...
    public function admin_options() {
        ...
        echo '<p><strong><a href="https://kabook.ir" target="_blank">پشتیبانی و مستندات افزونه</a></strong></p>';
